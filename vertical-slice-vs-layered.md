...
- `Shared/`の中だけを見ると、Interfaces→Services→Repositories→Modelsという**Layered Architectureそのものの層構成**になっている
...
│
├── Services/
│   ├── AuditLogService.cs        ← 全feature共通。Interfaceは無し
│   ├── Products/
│   │   ├── IProductService.cs
│   │   └── ProductService.cs
│   └── Orders/
